...
export default function Receitas({ dataInitial = [] }) {
  ...
  async function handleDelete(_id) {
    ...
    if (error) {
      logger.error('Erro ao tentar remover o modelo', {
        module: 'Modelos receitas - handleDelete',
        body: {
          _id
        }
      })
  ...
  return (
    <Menu title="Modelos de receitas">
      <div className="flex flex-col lg:flex-row gap-3 justify-between px-5 xl:px-10 py-8 items-start lg:items-center w-full">
        ...
        <div className="grid w-full xl:max-w-[250px] grid-cols-1 gap-4">
          <button
            onClick={() => replace('/modelos/receitas/cadastro')}
                    ...
                    <td className="border-b border-r text-left p-2">
                      <div className="flex gap-1">
                        <button
                          ...
                          onClick={() =>
                            replace(`/modelos/receitas/cadastro?id=${item._id}`)
                          }
                        >
                          <Icon.FaEdit size={13} />
                        </button>
...
export async function getServerSideProps(ctx) {
  ...
  try {
    const response = await api.get('/modelos?tipo=Receita')
    if (!response) {
