...
export default function Prontuarios({ dataInitial = [] }) {
  ...
  async function handleDelete(_id) {
    ...
    if (error) {
      logger.error('Erro ao tentar remover o modelo', {
        module: 'Modelos Prontuarios - handleDelete',
        body: {
          _id
        }
      })
  ...
  return (
    <Menu title="Modelos de prontuários">
      <div className="flex flex-col lg:flex-row gap-3 justify-between px-5 xl:px-10 py-8 items-start lg:items-center w-full">
        ...
        <div className="grid w-full xl:max-w-[250px] grid-cols-1 gap-4">
          <button
            onClick={() => replace('/modelos/prontuario/cadastro')}
                    ...
                    <td className="border-b border-r text-left p-2">
                      <div className="flex gap-1">
                        <button
                          ...
                          onClick={() =>
                            replace(
                              `/modelos/prontuario/cadastro?id=${item._id}`
                            )
                          }
                        >
                          <Icon.FaEdit size={13} />
...
export async function getServerSideProps(ctx) {
  ...
  try {
    const response = await api.get('/modelos?tipo=Prontuario')
    if (!response) {
    ...
    logger.error(err.response.data || err.message, {
      module: 'Modelos Prontuarios - SSR'
    })
    return {
      props: {
        dataInitial: []
