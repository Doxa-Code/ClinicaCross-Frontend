...
export default function AtestadosCadastro({ modelo = {} }) {
  const editor = useRef()
  ...
  async function handleSubmit(e) {
    ...
    if (error) {
      logger.error('Erro ao cadastrar atestado', {
        module: 'Cadastro de atestados - handleSubmit',
        body: {
          fields
        }
      })
    ...
    replace('/modelos/atestados')
  }

  return (
    <Menu title="Cadastro de Modelo de Atestado">
      <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-2 items-center">
          ...
          <button
            type="button"
            onClick={() => replace('/modelos/atestados')}
...
export async function getServerSideProps(ctx) {
  ...
  try {
    ...
    logger.error(err.response.data || err.message, {
      module: 'Cadastro de Modelos Atestados - SSR'
    })
    return {
      props: {
        modelo: {}
