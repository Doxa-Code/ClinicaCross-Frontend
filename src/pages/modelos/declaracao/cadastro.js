...
export default function declaracaoCadastro({ modelo = {} }) {
  const editor = useRef()
  ...
  async function handleSubmit(e) {
    ...
    if (error) {
      logger.error('Erro ao cadastrar declaração', {
        module: 'Cadastro de declarações - handleSubmit',
        body: {
          fields
        }
      })
    ...
    replace('/modelos/declaracao')
  }

  return (
    <Menu title="Cadastro de Modelo de declaracao">
      <form onSubmit={handleSubmit} className="p-8 flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-2 items-center">
          ...
          <button
            type="button"
            onClick={() => replace('/modelos/declaracao')}
...
export async function getServerSideProps(ctx) {
  ...
  try {
    ...
    logger.error(err.response.data || err.message, {
      module: 'Cadastro de Modelos Declaracoes - SSR'
    })
    return {
      props: {
        modelo: {}
