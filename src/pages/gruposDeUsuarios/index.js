...
export default function Grupos({ dataInitial = [] }) {
  const [data, mutate] = useState(dataInitial)
  const [grupos, setGrupos] = useState(dataInitial)
  const { replace } = useRouter()
  const { remove } = useFetch()

  function handleSearch(value) {
    if (value === '') {
      return mutate(grupos)
    }
    mutate(grupos.filter(item => item.nome.match(new RegExp(value, 'gim'))))
  }

  async function handleDelete(_id) {
    // eslint-disable-next-line no-unused-vars
    const [_, error] = await remove(`/grupos/${_id}`)
    if (error) return
    mutate(data.filter(item => item._id !== _id))
    setGrupos(grupos.filter(item => item._id !== _id))
    swal('Sucesso', 'Grupo removido com sucesso', 'success', {
      timer: 3000,
      buttons: false
    })
  }
    ...
    <Menu title="Grupos de usuários">
      <div className="flex flex-col lg:flex-row gap-3 justify-between px-5 xl:px-10 py-8 items-start lg:items-center w-full">
        ...
        <div className="grid w-full xl:max-w-[200px] grid-cols-1 gap-4">
          <button
            onClick={() => replace('/gruposDeUsuarios/cadastro')}
          ...
          >
            <Icon.FaUserLock size={18} /> Novo grupo
          </button>
        </div>
      </div>
                  ...
                  <tr key={key} className="text-center">
                    ...
                    <td className="gap-2 flex border-b border-r text-left px-3 p-2 justify-start items-center">
                      <button
                        ...
                        onClick={() =>
                          replace(`/gruposDeUsuarios/cadastro?id=${item._id}`)
                        }
                      >
                        <Icon.FaEdit size={13} />
                      </button>
