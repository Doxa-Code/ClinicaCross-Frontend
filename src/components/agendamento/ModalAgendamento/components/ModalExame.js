...
export default function ModalExame() {
  const { setShowExame, showExame, exame, readOnly } = useExameStore(
    state => state
  )
  ...
  async function handleSubmit(e) {
    ...
    const [response, error] = await create(
      `/pacientes/exame/${agendamento.paciente._id}`,
      {
        ...fields,
  ...
  return (
    <Modal closeWithEsc open={showExame} setOpen={setShowExame}>
      <div className="bg-white w-full max-w-4xl rounded-xl justify-center items-end">
        <div className="flex justify-between items-center p-5 rounded-t-xl text-amber-400 border-b border-amber-400">
          <span className="text-2xl font-semibold">
            {readOnly ? 'Visualizar exame' : 'Novo exame'}
          </span>
          <Icons.MdClose
            size={20}
        ...
        <form className="px-5 py-5 grid gap-4" onSubmit={handleSubmit}>
          ...
          >
            <div className="grid">
              ...
              <span className="text-xl">{exame.titulo}</span>
            </div>
            <div className="grid">
