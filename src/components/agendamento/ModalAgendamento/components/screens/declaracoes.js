...
export default function Declaracoes() {
  ...
  const { setShowDeclaracao, setDeclaracao, setReadOnly } = useDeclaracaoStore(
    state => state
  )
  const [declaracoes, setDeclaracoes] = useState([])

  useEffect(() => {
    setDeclaracoes(agendamento.paciente?.declaracao)
  }, [agendamento])

  function handleSearch(type) {
    return function (e) {
      if (!e.target.value) {
        return setDeclaracoes(agendamento.paciente.declaracao)
      }
      setDeclaracoes(
        agendamento.paciente.declaracao?.filter(item => {
          switch (type) {
            case 'titulo':
              return item.titulo
                ?.toLowerCase()
  ...
  return (
    <div className={`px-5 pt-4 gap-4 ${screen !== 'Declarações' && 'hidden'}`}>
