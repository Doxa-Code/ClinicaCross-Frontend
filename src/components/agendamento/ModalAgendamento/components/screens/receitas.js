...
export default function Receitas() {
  ...
  const { setShowReceita, setReceita, setReadOnly, setMedicamentos } =
    useReceitaStore(state => state)
  const { remove, fetch } = useFetch()
  const [receitas, setReceitas] = useState([])

  useEffect(() => {
    setReceitas(agendamento.paciente?.receita)
    handleLoad()
  }, [agendamento])

  async function handleLoad() {
    const [data, error] = await fetch('/medicamentos')
    if (error) return
  ...
  function handleSearch(type) {
    return function (e) {
      if (!e.target.value) {
        return setReceitas(agendamento.paciente.receita)
      }
      setReceitas(
        agendamento.paciente.receitas?.filter(item => {
          switch (type) {
            case 'titulo':
              return item.titulo
                ?.toLowerCase()
  ...
  async function handleDelete(id) {
    const [data, error] = await remove(
      `/pacientes/receita/${agendamento.paciente._id}/${id}`
    )
    if (error) return
  ...
  return (
    <div className={`px-5 pt-4 gap-4 ${screen !== 'Receitas' && 'hidden'}`}>
